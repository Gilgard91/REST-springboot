...
public class FCMService {
    ...
    public void sendMessageToTopic(NotificationRequest request) throws ExecutionException, InterruptedException {
        ...
        logger.info("Messaggio inviato al topic. Topic: " + request.getTopic() + ", " + response);
    }
    ...
    public void sendMessageToToken(NotificationRequest request)
        ...
        String response = FirebaseMessaging.getInstance().sendAsync(message).get();
        logger.info("Messaggio inviato. Device token: " + request.getToken() + ", " + response+ " msg "+jsonOutput);
    }
